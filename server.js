...
io.on('connection', socket => {
  ...
  socket.on('startMemoryTimer', () => {
    ...
    game.memoryEndsAt=Date.now()+1000;
    broadcast(game);
  });
  ...
  socket.on('confirmSelection',()=>{ const game=games.get(socket.data.gameCode); const user=userFor(game,socket); if(!game||!user||!['player','host'].includes(user.role)||game.phase!==PHASES.CONFIRM)return;
    ...
    cell.state='unavailable'; cell.revealedAt=Date.now(); game.revealEndsAt=null; game.firstCellRevealed=true; cell.timerEndsAt=null; game.currentCellId=null; game.phase=PHASES.WAITING; game.currentPlayerId=nextPlayer(game)?.id ?? null; broadcast(game);
  ...
  socket.on('forceEndGame',()=>{
    ...
    const {game}=ctx;
    game.closed=true;
    io.to(game.code).emit('gameClosed');
