...
io.on('connection', socket => {
  ...
  socket.on('confirmSelection',()=>{ const game=games.get(socket.data.gameCode); const user=userFor(game,socket); if(!game||!user||!['player','host'].includes(user.role)||game.phase!==PHASES.CONFIRM)return;
    ...
    cell.state='revealed'; cell.revealedAt=Date.now(); game.revealEndsAt=null; game.firstCellRevealed=true; cell.timerEndsAt=null; game.phase=PHASES.WAITING; game.currentPlayerId=nextPlayer(game)?.id ?? null; broadcast(game);
