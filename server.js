...
io.on('connection', socket => {
  ...
  socket.on('confirmSelection',()=>{ const game=games.get(socket.data.gameCode); const user=userFor(game,socket); if(!game||!user||!['player','host'].includes(user.role)||game.phase!==PHASES.CONFIRM)return;
    ...
    cell.state='revealed'; cell.revealedAt=Date.now(); game.revealEndsAt=Date.now()+30000; game.firstCellRevealed=true; cell.timerEndsAt=game.revealEndsAt; game.phase=PHASES.REVEAL; broadcast(game);
